        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 max-w-4xl mx-auto">
          {committeeMembers.map((member, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="glass rounded-lg p-4 sm:p-6 border border-gray-200 dark:border-white/10 text-center hover:border-red-500/30 transition"
            >
              <div className="inline-block px-2.5 py-1 sm:px-3 sm:py-1 bg-red-600/20 border border-red-500/50 rounded-full text-xs font-semibold text-red-700 dark:text-red-300 mb-3 sm:mb-4 uppercase tracking-wider">
                {member.name}
              </div>

              <h3 className="text-gray-900 dark:text-white font-bold text-sm sm:text-base lg:text-lg mb-1 sm:mb-2">{member.title}</h3>
