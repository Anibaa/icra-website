 ...
 * Run with: node scripts/download-flags.js
 */
...
const flagsDir = path.join(__dirname, '..', 'public', 'country-flags');
  ...
  return new Promise((resolve, reject) => {
    ...
    https.get(url, (response) => {
      if (response.statusCode === 200) {
        ...
        console.log(`✗ ${code} - ${name} (${response.statusCode})`);
...
// Download all flags with rate limiting
async function downloadAllFlags() {
  ...
  for (const code of codes) {
    await downloadFlag(code, countries[code]);
    completed++;
  ...
  console.log(`\n✓ Download complete! ${completed}/${codes.length} flags processed.`);
